...
Z architektonické a provozní perspektivy je LE integrována do výpočtů rozpočtu přenosové cesty a požadavků na výkon definovaných ve specifikacích jako 38.820 (pro NR) a 43.050 (pro scénáře RF systémů). Když základnová stanice (gNB v 5G NR nebo [eNB](/mobilnisite/slovnik/enb/) v LTE) vytváří paprsek, je signál váhován a přiváděn na každý anténní prvek. Hodnota LE pro každý prvek signál před jeho vyzářením utlumí. Systémoví návrháři a plánovači sítí musí s touto ztrátou počítat, aby zajistili, že vysílaný výkon splňuje regulatorní limity a poskytuje zamýšlené pokrytí. Ovlivňuje také kalibrační procedury pro anténní pole, protože rozdíly v LE mezi prvky mohou vést ke zkreslení tvaru paprsku a zvýšeným úrovním bočních laloků.
...
Historicky, jak se buněčné systémy vyvíjely od jednoduchých transceiverů s jednou anténou ke komplexním víceanténním systémům v LTE (od Rel-8 dále) a zejména s nástupem beamformingu velkého rozsahu v 5G NR, se potřeba detailního modelování RF parametrů stala klíčovou. Bez zohlednění ztrát na jednotlivých prvcích by vypočítaný zisk beamformingu a poměr signálu k šumu a rušení (SINR) byly nadhodnoceny. To by mohlo vést k poddimenzovaným sítím, mezerám v pokrytí a nesplnění cílů kvality služeb. Standardizací LE řeší 3GPP omezení předchozích zjednodušených modelů a poskytuje společný rámec pro specifikaci a testování vysokofrekvenčního výkonu anténních polí.
