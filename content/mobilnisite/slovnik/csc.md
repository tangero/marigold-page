...
Role CSC je ústřední pro paradigma síťového řezání definované od 3GPP Release 15 dále. Když CSC (např. automobilová společnost) potřebuje vyhrazený síťový řez pro služby spojených vozidel, odešle prostřednictvím CnF žádost o službu. Tato žádost, obsahující Profil služby síťového řezu, spustí v orchestračních systémech CSP proces vytvoření potřebných síťových funkcí napříč RAN, přenosovou sítí a core sítí k naplnění smlouvy o úrovni služeb (SLA). CSC pak získává průběžný přehled o výkonu řezu, včetně klíčových ukazatelů výkonu ([KPI](/mobilnisite/slovnik/kpi/)), jako je latence, propustnost a dostupnost, což umožňuje proaktivní správu kvality uživatelského zážitku jejich aplikace.
