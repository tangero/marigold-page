...
Network Control Protocol (NCP) je protokol řídicí roviny definovaný v raných vydáních 3GPP, primárně pro správu kontextů paketového datového protokolu (PDP) v sítích [GPRS](/mobilnisite/slovnik/gprs/) a UMTS. Působí mezi mobilní stanicí ([MS](/mobilnisite/slovnik/ms/)) a obslužným uzlem podpory GPRS (SGSN) k zřízení, změně a deaktivaci PDP kontextů, které jsou nezbytné pro IP datové relace. Protokol zajišťuje signalizaci pro správu relací, včetně vyjednávání parametrů kvality služby (QoS), přidělování IP adres a správy síťových zdrojů. Je klíčovou součástí vrstvy správy relací GPRS (SM) a spolupracuje se správou mobility ([MM](/mobilnisite/slovnik/mm/)) k zajištění plynulé datové konektivity.
...
Během provozu, když uživatel zahájí datovou relaci, MS odešle prostřednictvím NCP do SGSN zprávu Activate PDP Context Request. Tato zpráva obsahuje požadovaný APN a parametry QoS. SGSN následně komunikuje s bránovým uzlem podpory GPRS (GGSN) k vytvoření [GTP](/mobilnisite/slovnik/gtp/) tunelu a přidělení IP adresy. Po úspěšné alokaci zdrojů odešle SGSN zpět MS zprávu Activate PDP Context Accept, která potvrzuje parametry relace. NCP také zajišťuje změny, jako je přejednání QoS během relace, a deaktivaci při jejím ukončení. Jeho role je klíčová pro efektivní správu zdrojů a zajištění, aby datové relace splňovaly požadovanou úroveň služeb.

Klíčové součásti NCP zahrnují jeho sadu zpráv, stavové automaty pro správu relací a integraci s dalšími síťovými prvky, jako je GGSN a [HLR](/mobilnisite/slovnik/hlr/). Protokol podporuje různé typy PDP, včetně IPv4 a IPv6, a komunikuje s funkcemi autentizace a účtování. Ačkoli pozdější vydání vyvinula mechanismy správy relací, NCP položil základy signalizace řídicí roviny v paketově přepínaných sítích a ovlivnil následné protokoly, jako je správa relací EPS ([ESM](/mobilnisite/slovnik/esm/)) v LTE a 5G. Jeho návrh klade důraz na spolehlivost a flexibilitu, což umožnilo rané mobilní datové služby.
