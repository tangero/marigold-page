...
Z architektonického hlediska je CAI implementován v rámci struktury fyzického kanálu rádiového rozhraní UMTS, konkrétně je spojen s Dedicated Physical Control Channel ([DPCCH](/mobilnisite/slovnik/dpcch/)). DPCCH přenáší řídicí informace vrstvy 1, včetně pilotních bitů pro odhad kanálu, příkazů pro řízení vysílacího výkonu (TPC) a volitelně Transport Format Combination Indicator (TFCI). V určitých konfiguracích a scénářích jsou pro přenos CAI využita specifická pole nebo bitové vzorce v rámci rámcové struktury DPCCH. Síť (Node B) nastaví hodnotu CAI na základě výsledku svých interních algoritmů pro přidělování kanálů a kontrol dostupnosti zdrojů a poté ji vysílá k UE. Fyzická vrstva UE přijme a dekóduje CAI a předá indikaci stavu přidělení vyšším vrstvám ([MAC](/mobilnisite/slovnik/mac/) a [RRC](/mobilnisite/slovnik/rrc/)), které následně řídí chování při přenosu.
