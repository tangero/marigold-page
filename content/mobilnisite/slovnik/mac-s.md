...
Resynchronisation Authentication Code (MAC-S) je klíčový bezpečnostní mechanismus definovaný v rámci protokolu Authentication and Key Agreement ([AKA](/mobilnisite/slovnik/aka/)) pro systémy 3GPP, především UMTS a později rozšířený pro LTE a 5G. Funguje jako Message Authentication Code ([MAC](/mobilnisite/slovnik/mac/)) specificky generovaný k ochraně procedury resynchronizace. Tato procedura se spustí, když se pořadové číslo ([SQN](/mobilnisite/slovnik/sqn/)) používané v protokolu AKA rozchází mezi uživatelským zařízením (UE) a autentizačním centrem sítě (AuC), což je stav známý jako 'synchronizační selhání'. MAC-S je vypočteno AuC pomocí kryptografického algoritmu (např. MILENAGE) se vstupy zahrnujícími tajný klíč účastníka (K), náhodnou výzvu ([RAND](/mobilnisite/slovnik/rand/)), nové pořadové číslo (SQN<sub>new</sub>) a další parametry. Tento vypočtený MAC-S je následně odeslán do UE uvnitř tokenu [AUTS](/mobilnisite/slovnik/auts/), který je součástí zprávy Synchronisation Failure.
