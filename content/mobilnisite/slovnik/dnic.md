...
Historicky koncept DNIC přebíral adresování X.121 používané v legacy paketových sítích X.25, což odráželo přechodnou povahu raných mobilních datových služeb. Jeho účelem bylo přinést strukturu a jednoznačnou identifikaci do rodícího se ekosystému mobilních dat. Jak se sítě vyvíjely směrem k IP Multimedia Subsystem (IMS) a plně IP architekturám v LTE, potřeba takového specifického číselného identifikátoru zeslábla. APN zůstal kritický, ale jeho rozřešování a proces výběru brány se více integrovaly s [DNS](/mobilnisite/slovnik/dns/) a mechanismy založenými na politikách, což učinilo explicitní komponentu DNIC v moderních specifikacích 5G z velké části zastaralou.
