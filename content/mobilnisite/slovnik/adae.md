...
Application Data Analytics Enablement (ADAE) je rámec standardizovaný 3GPP pro usnadnění využívání analytických údajů generovaných sítí externími aplikacemi. Funguje jako služební schopnost v rámci 5G Core Network, konkrétně definovaná jako služba Network Exposure Function (NEF). Hlavním architektonickým principem je aplikační funkce (AF) v roli klienta, která zasílá požadavky na odběr analytických údajů službě ADAE hostované v NEF. Služba ADAE následně komunikuje s různými zdroji dat v síti, jako je Network Data Analytics Function (NWDAF), Unified Data Repository (UDR) nebo jiné síťové funkce (NF), aby shromáždila, zpracovala a doručila požadované analytické reporty zpět k AF.
