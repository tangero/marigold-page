...
Technický provoz zahrnuje procedury navazování a řízení relace. Pro odchozí mobilní relaci CSI s podporou CSI indikuje uživatelské zařízení (UE) svou schopnost CSI. Síť, často na základě rozhodnutí o politice v IMS, může rozdělit mediální komponenty: hlas v reálném čase je směrován přes nosič CS pomocí tradičního řízení hovoru (např. přes MSC), zatímco jiné mediální proudy (např. video) jsou navázány jako samostatné IP toky přes nosič paketového přepojování (PS) řízený IMS pomocí protokolu Session Initiation Protocol (SIP). IMS funguje jako kotva servisního řízení, zajišťuje, že účtování, doplňkové služby (jako přidržení hovoru nebo přesměrování) a servisní logika jsou konzistentně aplikovány napříč oběma doménami. To vyžaduje těsnou synchronizaci mezi signalizací SIP v IMS a signalizací ISUP/BICC v jádru CS.
