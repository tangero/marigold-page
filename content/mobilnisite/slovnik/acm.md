---
slug: "acm"
url: "/mobilnisite/slovnik/acm/"
type: "slovnik"
title: "ACM – Association for Computing Machinery"
date: 2026-01-01
abbr: "ACM"
fullName: "Association for Computing Machinery"
category: "Other"
segment: "Services"
canonical: "https://3gpp-explorer.com/glossary/acm/"
summary: "Association for Computing Machinery (ACM) je celosvětová vědecká a vzdělávací organizace zaměřená na rozvoj výpočetní techniky jako vědy a profese. Ačkoli se nejedná o technologii 3GPP, její publikace"
---

ACM je celosvětová vědecká a vzdělávací organizace, jejíž základní publikace a standardy z oblasti výpočetní techniky jsou odkazovány ve specifikacích 3GPP.

## Popis

Association for Computing Machinery (ACM) je největší světová vzdělávací a vědecká společnost v oblasti výpočetní techniky. Poskytuje zdroje pro rozvoj výpočetní techniky jako vědy a profese. ACM provozuje přední Digitální knihovnu oboru a slouží svým členům i profesionálům špičkovými publikacemi, konferencemi a kariérními zdroji. V kontextu specifikací 3GPP je na ACM odkazováno nikoli jako na síťovou funkci nebo protokol, ale jako na autoritativní zdroj ustálených výpočetních principů, algoritmů a klasifikačních systémů. Například technické zprávy a specifikace 3GPP mohou citovat ACM Computing Classification System (CCS) pro kategorizaci výzkumných oblastí nebo odkazovat na zásadní práce publikované v časopisech či sbornících ACM, které jsou základem algoritmů používaných v oblastech jako komprese dat, kryptografie zabezpečení nebo optimalizace sítí. Odkazy na ACM v dokumentech 3GPP (např. TR 21.905, TR 38.811) se typicky objevují v bibliografiích nebo normativních odkazech a slouží k uznání základní práce, na níž jsou určité technické implementace vybudovány. To zdůrazňuje interdisciplinární povahu telekomunikačního inženýrství, které se opírá o solidní základy informatiky. ACM Digital Library, rozsáhlý archiv plnotextových článků z časopisů, magazínů, konferenčních sborníků a zpravodajů ACM, slouží jako klíčový zdroj pro výzkumníky a inženýry vyvíjející algoritmy, které se následně standardizují. Přestože tedy ACM nedefinuje protokoly 3GPP, představuje širší akademický a profesní ekosystém, který dodává teoretické a praktické pokroky využívané standardizačními orgány. Porozumění její roli je klíčové pro pochopení původu a intelektuálních základů některých technických řešení zabudovaných v standardech mobilní komunikace.

## K čemu slouží

Účelem Association for Computing Machinery, založené v roce 1947, je rozvíjet umění, vědu, inženýrství a aplikace informačních technologií. Slouží k podpoře otevřené výměny informací, prosazování nejvyšších profesních a etických standardů a poskytuje platformu pro spolupráci odborníků a akademiků v oboru výpočetní techniky. V kontextu její citace v dokumentech 3GPP je účelem ACM poskytovat prověřený, recenzovaný zdroj základních znalostí. 3GPP jako standardizační orgán zaměřený na inženýrství často staví na ověřeném výzkumu v informatice, místo aby znovu vynalézal základní algoritmy. Odkazování na publikace ACM umožňuje 3GPP normativně poukázat na stabilní, externě spravované definice a metodologie, čímž zajišťuje technickou přesnost a vyhýbá se duplikaci úsilí při specifikaci detailů algoritmů na nízké úrovni. Tato praxe řeší omezení standardizačních orgánů, které se musí soustředit na systémovou architekturu a interoperabilitu, ale zároveň potřebují spoléhat na nejnovější pokroky z výzkumné komunity. Vytváří tak zásadní spojení mezi akademickým objevem a průmyslovou implementací a zajišťuje, že standardy mobilních sítí začleňují nejmodernější poznatky z příslušných výpočetních oborů.

## Klíčové vlastnosti

- Celosvětová profesní členská organizace pro výpočetní techniku
- Vydavatel prestižních recenzovaných časopisů a konferenčních sborníků
- Spravuje ACM Digital Library, komplexní zdroj literatury z oblasti výpočetní techniky
- Vyvíjí a spravuje ACM Computing Classification System (CCS)
- Pořádá významné mezinárodní konference napříč obory výpočetní techniky
- Stanovuje a propaguje etické kodexy pro profesionály v oboru výpočetní techniky

## Definující specifikace

- **TR 21.905** (Rel-19) — 3GPP Technical Terms and Definitions
- **TS 22.024** (Rel-19) — Advice of Charge (AoC) Supplementary Service
- **TS 23.018** (Rel-19) — Basic call handling in 3GPP CS domain
- **TS 23.087** (Rel-19) — User-to-User Signalling (UUS) Stage 2
- **TS 23.280** (Rel-20) — Common Architecture for Mission Critical Services
- **TS 24.173** (Rel-19) — Multimedia Telephony Service and Supplementary Services in IMS
- **TS 24.259** (Rel-19) — Personal Network Management (PNM) Protocol Details
- **TS 24.404** (Rel-7) — Communication Diversion Services (CDIV)
- **TS 24.504** (Rel-8) — Communication Diversion Services Stage 3
- **TS 24.604** (Rel-19) — Communications Diversion (CDIV) Protocol Spec
- **TS 26.804** (Rel-19) — 5G Media Streaming Extensions Study
- **TS 31.121** (Rel-18) — UICC-terminal interface test specification
- **TS 38.811** (Rel-15) — Study on NR Support for Non-Terrestrial Networks

---

📖 **Anglický originál a plná specifikace:** [ACM na 3GPP Explorer](https://3gpp-explorer.com/glossary/acm/)
