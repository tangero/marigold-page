...
Mobile Country Code (MCC) je klíčový, standardizovaný identifikátor v rámci globálního telekomunikačního číslovacího plánu definovaného Mezinárodní telekomunikační unií ([ITU](/mobilnisite/slovnik/itu/)) a přijatého organizací 3GPP. Skládá se ze tří desítkových číslic (např. 310 pro Spojené státy, 262 pro Německo) a slouží jako primární zeměpisný identifikátor pro mobilního účastníka. MCC je trvale zakódován v SIM kartě účastníka jako úvodní segment 15místného International Mobile Subscriber Identity ([IMSI](/mobilnisite/slovnik/imsi/)), který má formát MCC-MNC-MSIN. Tato struktura zajišťuje, že každý mobilní účastník na světě má globálně jedinečný identifikátor.
...
MCC spolupracuje s Mobile Network Code (MNC) a společně tvoří PLMN ID (Public Land Mobile Network Identity), které jednoznačně identifikuje konkrétní síť operátora v rámci země. Síťové elementy jako Mobile Switching Center ([MSC](/mobilnisite/slovnik/msc/)), Serving [GPRS](/mobilnisite/slovnik/gprs/) Support Node (SGSN) a Mobility Management Entity ([MME](/mobilnisite/slovnik/mme/)) používají PLMN ID (a tedy i MCC) pro funkce jako autentizace, registrace, předávání hovorů mezi buňkami (handover) a účtování. Při mezinárodním roamingu, když se účastník pokusí zaregistrovat v navštívené síti (VPLMN), použije VPLMN MCC z IMSI účastníka k identifikaci domovské země a následně dotazuje příslušnou signalizační síť mezi operátory (jako SS7 nebo Diameter-based IPX), aby kontaktovala domovskou síť (HPLMN) pro autentizaci a autorizaci služeb.
