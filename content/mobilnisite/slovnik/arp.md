...
Allocation and Retention Priority (ARP) je klíčový parametr kvality služeb (QoS) definovaný v architektuře 3GPP, konkrétně v rámci frameworku Policy and Charging Control (PCC). Funguje jako skalární hodnota, která typicky zahrnuje úroveň priority (1–15, přičemž 1 je nejvyšší), příznak schopnosti přerušení (pre-emption capability) a příznak zranitelnosti vůči přerušení (pre-emption vulnerability). ARP není parametrem na úrovni beareru používaným pro dynamické plánování (jako [QCI](/mobilnisite/slovnik/qci/)), ale spíše parametrem na úrovni předplatného nebo relace uplatňovaným při zřizování a správě životního cyklu beareru. Jeho primární funkce je vyvolána funkcí Policy and Charging Rules Function (PCRF) a vynucována funkcí Policy and Charging Enforcement Function (PCEF) v jádru sítě, ve spolupráci s Radio Access Network (RAN) během správy rádiových prostředků (RRM).
