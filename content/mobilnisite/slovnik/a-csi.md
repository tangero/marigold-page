...
Konfigurace hlášení A-CSI je stanovena signalizací vyšší vrstvy [RRC](/mobilnisite/slovnik/rrc/) (CSI-ReportConfig), která definuje podrobné parametry pro hlášení. To zahrnuje množství informací v hlášení CSI (např. indikátor kvality kanálu ([CQI](/mobilnisite/slovnik/cqi/)), indikátor předkódovací matice (PMI), indikátor hodnosti (RI), indikátor vrstvy ([LI](/mobilnisite/slovnik/li/)) a indikátor zdroje [CSI-RS](/mobilnisite/slovnik/csi-rs/) ([CRI](/mobilnisite/slovnik/cri/))), přidruženou sadu zdrojů CSI-RS pro měření a časové chování nastavené na 'aperiodic'. Když se gNB rozhodne, že je vyžadováno hlášení A-CSI, odešle DCI, které obsahuje pole žádosti o CSI. Toto pole odkazuje na jeden nebo více spouštěcích stavů (trigger states), z nichž každý odkazuje na sadu CSI-ReportConfigs. Po přijetí a dekódování podnětu UE změří nakonfigurované referenční signály pro informace o stavu kanálu (CSI-RS), vypočítá požadované metriky CSI a multiplexuje užitečná data hlášení CSI do přiděleného zdroje PUSCH, jak je uvedeno ve stejném povolení DCI.
