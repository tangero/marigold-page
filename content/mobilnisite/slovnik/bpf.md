...
Pásmový filtr (Band Pass Filter – BPF) je lineární, časově invariantní filtr charakterizovaný svým propustným pásmem, nepropustným pásmem, střední frekvencí, šířkou pásma a profilu útlumu. V kontextu 3GPP rádiových systémů funguje v rámci RF předkoncového stupně transceiverů. Architektonicky je umístěn za anténou a šumovým předzesilovačem ([LNA](/mobilnisite/slovnik/lna/)) v přijímacím řetězci a před výkonovým zesilovačem (PA) v vysílacím řetězci. Jeho hlavní úlohou je propustit pouze frekvence určeného komunikačního kanálu a zároveň potlačit mimopásmové signály, včetně rušení z přilehlých kanálů, harmonických složek a šumu. Tato selektivní filtrace je nezbytná pro zachování integrity signálu a prevenci desenzibilizace přijímače.
