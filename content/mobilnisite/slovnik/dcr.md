...
Historický kontext pro přijetí DCR v rámci 3GPP vychází z potřeby vybrat a optimalizovat řečové kodeky pro 3G UMTS a následující generace. Když 3GPP vyvíjelo rodinu kodeků Adaptive Multi-Rate ([AMR](/mobilnisite/slovnik/amr/)), potřebovalo spolehlivou metodu pro hodnocení jeho výkonu za chybových rádiových podmínek sítí WCDMA. DCR umožnila inženýrům systematicky měřit, jak různé režimy AMR, techniky maskování chyb a strategie adaptace rádiového spoje ovlivňují kvalitu poslechu při výskytu výmazů rámců (simulujících chyby rádiových bloků). To byl významný pokrok oproti pouhému měření bitové chybovosti ([BER](/mobilnisite/slovnik/ber/)) nebo míry výmazů rámců (FER), protože přímo korelovala technické parametry s lidským vnímáním.
