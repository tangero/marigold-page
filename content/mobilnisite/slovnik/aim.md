...
Application Information Model (AIM) je komplexní datový model definovaný v rámci 3GPP management framework, který specifikuje strukturu, atributy, vztahy a chování informací souvisejících s aplikacemi. Slouží jako abstraktní reprezentace aplikací nasazených v telekomunikační síti nebo s ní interagujících, zachycující jak statické charakteristiky, tak dynamické stavové informace. Model je typicky vyjádřen pomocí standardizovaných modelovacích jazyků, jako je UML (Unified Modeling Language), a je implementován prostřednictvím management rozhraní, jako jsou ta definovaná v rámci 3GPP Management Object Model (MOM) framework.
...
Historicky zavedení AIM v R99 korespondovalo s nástupem sofistikovanějších mobilních služeb přesahujících základní hlasové hovory, včetně raných datových služeb a aplikací s přidanou hodnotou. Jak se sítě vyvíjely směrem k 3G a pozdějším generacím, potřeba standardizované správy aplikací se stávala stále kritičtější pro podporu rostoucího ekosystému aplikací a služeb třetích stran. AIM řešil omezení předchozích ad-hoc přístupů tím, že poskytl framework schopný škálovat s rostoucí diverzitou a složitostí síťových aplikací při zachování zpětné kompatibility s existujícími management systémy.
