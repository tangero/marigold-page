...
MBW dále podporuje vývoj pokročilých funkcí, jako je agregace nosných a dynamické sdílení spektra. Specifikací měřicích šířek pásma pro každou komponentní nosnou a agregovanou šířku pásma umožňuje ověření složitých RF scénářů. Parametr se také přizpůsobuje novým případům užití, jako je integrovaný přístup a přenosová síť (IAB) a mimozemské sítě (NTN), kde vznikají jedinečné konfigurace šířky pásma. Celkově MBW zajišťuje, že zařízení splňují jak technické, tak regulační požadavky, což usnadňuje globální interoperabilitu a efektivní využití spektra.
