...
Úroveň aktivní řeči (Active Speech Level – ASL) je technický parametr definovaný 3GPP pro kvantifikaci průměrné úrovně výkonu řeči během aktivních mluvních úseků, s vyloučením období ticha a šumu na pozadí. Měří se v decibelech vzhledem k plnému rozsahu digitálního signálu (dBov) nebo vzhledem ke stanovenému referenčnímu bodu (dBm0). Výpočet zahrnuje segmentaci audio signálu, identifikaci intervalů aktivní řeči pomocí algoritmů detekce hlasové aktivity (Voice Activity Detection – VAD) a výpočet střední kvadratické (RMS) hodnoty výkonu v těchto intervalech. Tento proces zajišťuje, že k měření úrovně přispívají pouze složky řeči, čímž poskytuje spolehlivou metriku hlasitosti řeči nezávislou na tichu nebo šumu.
