...
Technická implementace zahrnuje koordinaci několika síťových prvků. Serving-Call Session Control Function (S-CSCF) zpracovává SIP signalizaci pro požadavky 3PTY, zatímco Media Resource Function ([MRF](/mobilnisite/slovnik/mrf/)) poskytuje schopnosti míchání médií pro kombinaci zvukových proudů od všech tří účastníků. User Equipment (UE) musí podporovat specifické SIP metody a hlavičky pro zahájení a správu relací 3PTY, včetně schopnosti stávající hovory přerušit (hold) během navazování nových spojení. Síť udržuje pro každého účastníka samostatný stav dialogu, přičemž navenek prezentuje jednotnou konferenci.
