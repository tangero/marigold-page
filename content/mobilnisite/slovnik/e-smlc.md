...
E-SMLC podporuje více polohovacích technik. Pro LTE to zahrnuje Observed Time Difference of Arrival ([OTDOA](/mobilnisite/slovnik/otdoa/)), která využívá referenční signály z více eNodeB; Assisted Global Navigation Satellite System ([A-GNSS](/mobilnisite/slovnik/a-gnss/)), která poskytuje pomocná data přijímači [GNSS](/mobilnisite/slovnik/gnss/) v UE; Enhanced Cell ID ([E-CID](/mobilnisite/slovnik/e-cid/)), která využívá časový předstih a měření signálu; a Uplink Time Difference of Arrival (UTDOA). Pro 5G NR podporuje podobné metody jako [DL-TDOA](/mobilnisite/slovnik/dl-tdoa/) (Downlink TDOA) a Multi-RTT (Round Trip Time). E-SMLC komunikuje s UE přes protokol [LPP](/mobilnisite/slovnik/lpp/) (LTE Positioning Protocol) nebo jeho NR protějšek, NRPPa, za účelem získání měření nebo doručení pomocných dat. Také komunikuje s základnovými stanicemi (eNodeB/gNB) a v případě potřeby se Secure User Plane Location (SUPL) Enabled Location Server (SLP) přes protokol LPPa, aby získal informace o buňce nebo nakonfiguroval polohovací referenční signály.
