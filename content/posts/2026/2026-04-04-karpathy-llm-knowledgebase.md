...
Andrej Karpathy - člověk, který stál u zrodu GPT - zveřejnil minulý týden Twitter příspěvek, který si za čtyři dny záložkovalo přes 82 000 lidí. Třináct milionů zobrazení. To není virální hype. Trefil něco, co lidi sami cítí, ale neuměli to pojmenovat. O co jde? Jak používat LLM k budování znalostní báze.
...
Stručně: přestal používat AI hlavně na kód. Místo toho ho používá na budování osobní znalostní báze. Pojďme to okopírovat, protože je to dobré. A rozvést, protože je kam.
...
Jeho wiki na jedno téma má teď ~100 článků a ~400 000 slov. Bez RAG, bez vektorové databáze. Jen flat adresář `.md` souborů v Obsidianu. Tady se zadrhávám já - poznámek mám kolem 50 000, hlavně k 3GPP normám.
...
AI tu přestává být nástroj na psaní nebo kódování a stává se **osobním knihovníkem a analytikem**.
...
**„Není to jako NotebookLM?”** - několik lidí srovnává s Google NotebookLM nebo Claude Projects. Rozdíl tu ale je, a podstatný: Karpathyho systém máte u sebe, offline a rozšiřitelný, a LLM v něm strukturu aktivně udržuje a rozvíjí, ne jen odpovídá na dotazy.
...
A to je vlastně to hlavní: **s agenty nesdílíte kód, sdílíte myšlenku.** Kód si každý vygeneruje sám, přizpůsobený vlastním potřebám. Za den po zveřejnění se v komentářích objevilo přes deset hotových implementací - od Go binárky přes Claude Code skill po voice-first verzi přes Telegram.
...
Podstatné je ale napsat kvalitní `CLAUDE.md` - schema soubor s instrukcemi, jak má LLM wiki strukturovat. **Tohle je 80 % hodnoty celého systému.** Definujte v něm formát článků, jak pojmenovávat soubory, jak propojovat koncepty, co dělat s protichůdnými informacemi. Příklad:
...
Pravidelné health checks nevynechávejte - bez nich se rozpory a nepřesnosti kumulují. Jeden uživatel v gist komentářích nastavil automatický noční lint přes naplánovaný prompt - wiki se tak „opravuje sama”.
...
To je rozdíl mezi člověkem, který hledá v Googlu, a člověkem, který má osobního výzkumného asistenta - ten pro něj celé měsíce čte a syntetizuje.
