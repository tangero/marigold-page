...
### 2. Tokenizace
Jakmile je prompt zadán, prvním krokem je tokenizace. Tokenizace rozděluje text na menší jednotky zvané [tokeny](/ai/tokeny-versus-slova/). Tyto [tokeny](/ai/tokeny-versus-slova/) mohou být slova, části slov, nebo dokonce jednotlivé znaky, v závislosti na použitém tokenizačním algoritmu. Ve [zvláštním článku vysvětluji](/ai/tokenizace-textu/), jaký je rozdíl mezi slovy a tokeny a proč se mechanismus tokenů používá. 
...
Nyní přichází na řadu jádro LLM - *masivní neuronová síť*, obvykle založená na architektuře *[Transformátorů](/ai/transformatory/)*. Tato síť se skládá z mnoha vrstev a může mít miliardy parametrů.
