...
### Changed
- Jazyková humanizace článku o Karpathym (`content/posts/2026/2026-04-04-karpathy-llm-knowledgebase.md`): odstranění AI artefaktů bez změny stylu — 70 em dashů (—) nahrazeno krátkou pomlčkou, přeformulována AI klišé („Posun je zásadní", „Rozdíl je klíčový", „To je meta-insight", kalk „jiný příběh", opakovaný kontrast „nutnost, ne volba/luxus"), dočištěny rozepsané autorské vsuvky bez diakritiky
...
- Doplnění téhož článku o redakční poznámku „Aktualizováno v červenci 2026" na začátku (odkazuje na tři místa s novinkami) a novou sekci „Má to pro tebe vůbec smysl?" — úvaha o smyslu second brainu (námitka „hřbitov poznámek", kdy je Google+LLM rychlejší volba, jednotlivec jako nástroj na syntézu vs. firma jako infrastruktura), sladěná s anti-RAG tezí článku (kompilace kontextu, ne RAG nad poznámkami)
...
### Added
- Slovník 3GPP: rozšíření architektonických schémat z 13 na **24 pojmů**. Diagram je pevná 5G SBA topologie, kde se vysvětlovaný pojem zvýrazní. Nově: CHF, UE, DN, RAN (uzly už v topologii) + SEPP, NWDAF, BSF, UDSF, SCP, N3IWF, TNGF (doplňkové SBA NF). SBA pás se při >7 uzlech zalomí na dvě řady (interleave — horní řada v mezerách spodní, spojky nekříží boxy), zdroj v 3gpp-explorer2 `render-network.ts`/`network-architecture.ts`
- Slovník 3GPP: automatické napojení architektonických SVG diagramů na generátor slovníku — `generate-marigold-md.ts` (v 3gpp-explorer2) je nově „diagram-aware": pro 13 pojmů 5G Core (5gc, amf, smf, upf, nssf, nrf, udm, udr, ausf, nef, pcf, af, s-nssai) vyexportuje české SVG ze zdrojového manifestu (`data/glossary-diagrams/manifest.json`, pole `svgCs`) do `static/assets/slovnik/<slug>.svg` a zároveň přidá `infografika` + `infografikaAlt` do front matteru. Dřív se SVG i pole `infografika` udržovaly ručně, takže každé přegenerování MD je smazalo (tichý rozpad). Layout `single.html` čte `infografika` přednostně, jinak fallback `fileExists` podle názvu — obojí teď konzistentní
