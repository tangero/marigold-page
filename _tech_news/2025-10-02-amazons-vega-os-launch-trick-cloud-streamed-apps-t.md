---
category: cloudové aplikace
companies:
- Amazon
date: '2025-10-02 15:30:00'
description: Amazon bude streamovat Android aplikace z cloudu, aby zajistil, že jeho
  nový dongle Fire TV založený na Veze bude mít při spuštění k dispozici všechny potřebné
  aplikace a služby.
importance: 4
layout: tech_news_article
original_title: 'Amazon’s Vega OS launch trick: cloud-streamed apps - The Verge'
publishedAt: '2025-10-02T15:30:00+00:00'
slug: amazons-vega-os-launch-trick-cloud-streamed-apps-t
source:
  emoji: ⚡
  id: the-verge
  name: The Verge
title: 'Trik při spuštění operačního systému Vega od Amazonu: cloudově streamované
  aplikace'
url: https://www.theverge.com/column/790144/amazons-vega-os-streaming-cloud
urlToImage: https://platform.theverge.com/wp-content/uploads/sites/2/2025/09/fire-tv-stick.jpeg?quality=90&strip=all&crop=0%2C3.4926328102429%2C100%2C93.014734379514&w=1200
---

Amazon bude streamovat Android aplikace z cloudu, aby zajistil, že jeho nový dongle Fire TV založený na Veze bude mít při spuštění k dispozici všechny potřebné aplikace a služby.

[Číst původní článek](https://www.theverge.com/column/790144/amazons-vega-os-streaming-cloud)

**Zdroj:** ⚡ The Verge
