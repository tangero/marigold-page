...
- AI
- Umělá inteligence
content_hash: 8e4a6feb21413fcf06cd35a7797f5d62
hide: true
lang: en
layout: post
original_lang: cs
...
Let's take an example. For example, I use a system for project logging where I write down what I've done, followed by a hash of the name of the project or work. For example, "I order a new server #marigold". I write that down in Excel. Now I want to have a web service where firstly I note this down, secondly I can list all these notes. It's a relatively simple application for our AI programming example.

**So let's be clear about the brief for this app:**
- Data is entered via a web form in any short text format and may include a hashtag, a word beginning with the # character.
...
- **Flask:** is a lightweight web framework that is simple to use, allows for rapid prototyping, and provides flexibility in application creation. Flask is very well documented, making it easy to develop and troubleshoot.
- **Wide support:** Python and Flask have a large community and a number of libraries that can simplify your work (for example, for managing users, working with databases, or generating graphs and word clouds).
- **Flexibility:** Flask is very modular, allowing easy integration with different databases (e.g. SQLite for easy deployment) and front-end frameworks.
...
#### Disadvantages:
- **Performance:** Flask isn't as powerful as other full-featured frameworks like Django or Node.js, but for a little-used service, this shouldn't be a problem.
- **Features:** Flask doesn't provide as many built-in features as more robust frameworks (like Django), which means you'll have to implement some features yourself.
...
#### Benefits:
- **Easy Deployment:** Heroku offers easy deployment of applications directly from GitHub or via command line, which is convenient for beginners.
- **Free:** Heroku offers a free plan, which is suitable for small and underutilized applications. Paid plans are available for larger scaling.
...
- **Databases:** Heroku offers built-in support for PostgreSQL databases, giving you a scalable and reliable database right on the platform.

#### Disadvantages:
- **Limited resources on the free plan:** Heroku's free plan has a limitation on the number of hours you can run your application per month and may not be suitable for applications with high traffic or large databases.
- **Inactivity startup:** Applications deployed on the free plan may sleep after inactivity and may have slower response times when restarted.

### Alternative options

- **Vercel or Netlify:** These platforms are mainly focused on the front-end, but can also be used for full-stack applications using serverless features. They are also very beginner friendly and have free plans.
- **Render:** An alternative to Heroku that offers similar deployment options with a simple user interface and free plans.

### Summary

I recommend using Python with Flask to develop your web application. This approach will give you readable and flexible code that is easy to understand for beginners. For hosting, I would recommend Heroku as it offers easy deployment and a free plan suitable for smaller projects.

**a.** Want to proceed with implementing the backend of your application using Flask?  
...
The AI is quite flexible on this, you can copy a very long error log to it and it will figure it out. It will explain where the error occurred and suggest an adjustment you need to make.
